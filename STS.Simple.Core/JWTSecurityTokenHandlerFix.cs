...
namespace STS.Simple.Core
{
    public class JWTSecurityTokenHandlerFix : JwtSecurityTokenHandler
    {
        ...
        private SecurityKey GetSecurityKey(string thumbprint)
        {
            var certificate = CertificateHelper.GetCertificate(StoreName.My,StoreLocation.LocalMachine,X509FindType.FindByThumbprint,thumbprint);
            if (certificate == null)
            {
                return null;
            }
